...
public class KafkaProducerPartition {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
         ...
         */
         props.put("partitioner.class", "com.intsmaze.kafka.partition.TestPartitioner");
//        props.put("partitioner.class", "org.apache.kafka.clients.producer.internals.DefaultPartitioner");// 默认值

        Producer<String, String> producer = new KafkaProducer<>(props);

        for (int i = 0; i < 13; i++) {
            Future<RecordMetadata> future = producer.send(new ProducerRecord<String, String>("kafka-test", Integer.toString(i)));
            future.get();
        }
    }
}
