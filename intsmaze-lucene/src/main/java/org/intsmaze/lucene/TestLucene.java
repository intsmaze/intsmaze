...
public class TestLucene {
    ...
    public static Document article2Document(Article article) {
        ...
        doc.add(new Field("id", article.getId()+"",
...
//        里面也有很多的字段（数据整理成结构化数据放到一个索引文档中的，比如非结构化数据网页，就会有字段存储网页内容，有字段存储网页链接，然后把这些整理成一起放到索引文档中）
        ...
        Field field = new IntField("id", 1, Field.Store.YES);
        Field title = new StringField("title",
                "洋洋哥，本科，软件工程专业，国家软考：程序员，软件设计师，软件架构师。", Field.Store.YES);
        Field content = new TextField("content",
                "一、《中国银行业监督管理委员会关于印发<农村信用社省（自治区、直辖市）联合社管理暂行规定>的通知》（银监发〔2003〕14号）等499件主要规范性文件（见附件1），继续有效。", Field.Store.YES);
        Field content1 = new TextField("content",
                "二、《中国银行业监督管理委员会关于印发<农村商业银行管理暂行规定>和<农村合作银行管理暂行规定>的通知》（银监发〔2003〕10号）等50件规范性文件（见附件2），自公告之日起废止。", Field.Store.YES);
        Field content2 = new TextField("content",
                "三、《中国银行业监督管理委员会关于加强信托投资公司风险监管防范交易对手风险的通知》（银监发〔2004〕93号）等34件规范性文件（见附件3），自公告之日起失效。", Field.Store.YES);
        doc.add(field);
        doc.add(title);
        doc.add(content);
        doc.add(content1);
